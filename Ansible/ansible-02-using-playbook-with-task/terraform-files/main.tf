...
resource "null_resource" "config" {
  ...
  connection {
    ...
    type = "ssh"
    user = "ec2-user"
    private_key = file("~/.ssh/xxxxxxxxxxxxxx.pem")
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "echo node1 ansible_host=${aws_instance.nodes[1].private_ip} ansible_ssh_private_key_file=~/xxxxxxxxxxxxxx.pem ansible_user=ec2-user >> inventory.txt",
      "echo node2 ansible_host=${aws_instance.nodes[2].private_ip} ansible_ssh_private_key_file=~/xxxxxxxxxxxxxx.pem ansible_user=ec2-user >> inventory.txt",
      "echo [ubuntuservers] >> inventory.txt",
      "echo node3 ansible_host=${aws_instance.nodes[3].private_ip} ansible_ssh_private_key_file=~/xxxxxxxxxxxxxx.pem ansible_user=ubuntu >> inventory.txt",
