...
resource "aws_instance" "amazon-linux-2" {
  ...
  count = 3
  key_name = "xxxxxxxxxxxxxxxxxxxx" ####### CHANGE HERE WITHOUT .pem  #######
...
resource "aws_instance" "ubuntu" {
  ...
  instance_type = "t2.micro"
  key_name = "xxxxxxxxxxxxxxxxxxx" ####### CHANGE HERE WITHOUT .pem #######
  security_groups = ["ansible-session-sec-gr"]
...
resource "null_resource" "config" {
  ...
  connection {
    ...
    type = "ssh"
    user = "ec2-user"
    private_key = file("~/.ssh/xxxxxxxxxxxxxxxxxxxxx") ####### CHANGE HERE WITH .pem #######
    }
